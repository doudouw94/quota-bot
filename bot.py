            ...
            with conn.cursor() as c:
                c.execute("""
                    SELECT
                        ...
                        STRING_AGG(DISTINCT q.subtype || ' x' || SUM(CASE WHEN q.subtype IS NOT NULL THEN q.quantity ELSE 0 END) OVER (PARTITION BY a.user_id, q.subtype), ', ') FILTER (WHERE q.type = 'Speedo') as speedo_detail
